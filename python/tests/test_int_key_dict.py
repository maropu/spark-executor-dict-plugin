...
class IntKeyDictTests(ReusedSQLTestCase):
    ...
    def conf(cls):
        return SparkConf() \
            ...
            .set("spark.files", "{}/int_key_dict.db".format(os.getenv("DICT_TESTDATA"))) \
            .set("spark.plugins.executorDict.port", "8001") \
            .set("spark.plugins.executorDict.keyType", "int")

    @classmethod
    def setUpClass(cls):
        super(IntKeyDictTests, cls).setUpClass()
    ...
    def test_basics(self):
        ...
        def _udf(x):
            client = DictClient(port=8001)
            return str(client.lookup_by_i32(x))

        df = self.spark.range(4).selectExpr("CAST(id AS INT) id")
    ...
    def test_retry_errors(self):
        ...
        self.assertRaises(RuntimeError, lambda: client.lookup_by_i32(1))
